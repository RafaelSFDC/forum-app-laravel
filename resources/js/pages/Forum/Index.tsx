    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-background">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0">
                            <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-4 lg:space-x-6">
                                <div className="flex flex-col">
                                    <h1 className="text-lg sm:text-xl font-bold text-foreground">Fórum</h1>
                                    {currentSearch && (
                                ...
                                <div className="flex items-center space-x-2 sm:space-x-3">
                                    ...
                                    <div className="flex space-x-1 bg-muted rounded-full p-0.5 sm:p-1">
                                        {[
                ...
                <div className="max-w-7xl mx-auto px-2 sm:px-4 py-3 sm:py-6 bg-background min-h-screen">
                    <div className="grid grid-cols-1 lg:grid-cols-4 gap-3 sm:gap-6">
                        {/* Sidebar com tópicos */}
                        <div className="lg:col-span-1 order-2 lg:order-1">
                            <Card className="bg-card border-border shadow-sm">
                                <CardHeader className="pb-2 sm:pb-3 px-3 sm:px-6 pt-3 sm:pt-6">
                                    <h3 className="font-semibold text-sm sm:text-base text-card-foreground">Tópicos</h3>
                                </CardHeader>
                            ...
                            {posts.data.length === 0 ? (
                                <Card className="bg-card border-border">
                                    <CardContent className="py-8 sm:py-12 text-center px-4">
                                        <p className="text-muted-foreground text-sm sm:text-base">Nenhum post encontrado.</p>
                                    </CardContent>
                                </Card>
                            ) : (
                                posts.data.map((post) => (
                                    <Card key={post.id} className="bg-card border-border hover:shadow-md transition-all duration-200 hover:border-border/80">
                                        <CardContent className="p-0">
                                            <div className="flex">
                                                {/* Botões de votação */}
                                                <div className="flex flex-col items-center space-y-0.5 sm:space-y-1 bg-muted p-1.5 sm:p-2 rounded-l-lg">
                                                    <Button variant="ghost" size="sm" className="p-0.5 sm:p-1 h-5 w-5 sm:h-6 sm:w-6 hover:bg-accent rounded">
                                                        <ArrowUp className="h-2.5 w-2.5 sm:h-3 sm:w-3" />
                                                    </Button>
                                                    <span className={cn(
                                                        "text-xs font-bold leading-none",
                                                        post.votes_count > 0 ? "text-primary" :
                                                        post.votes_count < 0 ? "text-accent-foreground" : "text-muted-foreground"
                                                    )}>
                                                        {post.votes_count}
                                                    </span>
                                                    <Button variant="ghost" size="sm" className="p-0.5 sm:p-1 h-5 w-5 sm:h-6 sm:w-6 hover:bg-accent rounded">
                                                ...
                                                <div className="flex-1 min-w-0 p-2 sm:p-4">
                                                    <div className="flex flex-wrap items-center gap-1 sm:gap-2 mb-1.5 sm:mb-2">
                                                        ...
                                                        <span className="text-xs text-muted-foreground">por u/{post.user.name}</span>
                                                        <span className="text-xs text-muted-foreground hidden sm:inline">•</span>
                                                        <span className="text-xs text-muted-foreground">{formatTimeAgo(post.created_at)}</span>
                                                        {post.is_pinned && (
                                                            <>
                                                                <span className="text-xs text-muted-foreground hidden sm:inline">•</span>
                                                                <Badge variant="outline" className="text-xs bg-chart-1/20 text-chart-1 border-chart-1/30 px-1.5 py-0.5">Fixado</Badge>
                                                            </>
                                                        )}
                                                    </div>

                                                    <h3 className="font-medium text-sm sm:text-lg mb-1.5 sm:mb-2 text-card-foreground hover:text-primary cursor-pointer transition-colors line-clamp-2">
                                                    ...
                                                    <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs text-muted-foreground">
                                                        <div className="flex items-center space-x-1 hover:bg-accent hover:text-accent-foreground rounded px-1.5 sm:px-2 py-1 cursor-pointer transition-colors">
